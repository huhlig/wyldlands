...
impl GoapAction {
    ...
    pub fn preconditions_met(&self, world_state: &WorldState) -> bool {
        self.preconditions
            .iter()
            .all(|(key, &value)| world_state.get(key).copied().unwrap_or(false) == value)
    }
...
impl GoapPlanner {
    ...
    pub fn update(&mut self) -> bool {
        ...
        if let Some(satisfied_goal) = self
            .goals
            .iter()
...
mod tests {
    ...
    fn test_goap_planner_multi_step_plan() {
        ...
        // Goal: Be inside
        let goal = GoapGoal::new("be_inside", "Be inside", 10).with_condition("inside", true);
        planner.add_goal(goal);
    ...
    fn test_goap_planner_choose_cheaper_path() {
        ...
        let goal =
            GoapGoal::new("reach_goal", "Reach the goal", 10).with_condition("at_goal", true);
        planner.add_goal(goal);
