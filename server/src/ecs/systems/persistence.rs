...
impl PersistenceSystem {
    ...
    pub fn serialize_entity(
        ...
        world: &GameWorld,
        entity: EcsEntity,
    ) -> Result<SerializedEntity, String> {
        ...
        if let Ok(entity_type) = world.get::<&EntityType>(entity) {
            components.insert(
                ...
                serde_json::to_value(&*entity_type).unwrap(),
            );
        }
        ...
        if let Ok(containable) = world.get::<&Containable>(entity) {
            components.insert(
                "containable".to_string(),
                serde_json::to_value(&*containable).unwrap(),
            );
        }
        ...
        if let Ok(personality) = world.get::<&Personality>(entity) {
            components.insert(
                ...
                serde_json::to_value(&*personality).unwrap(),
            );
        }
        ...
        if let Ok(combatant) = world.get::<&Combatant>(entity) {
            components.insert(
                "combatant".to_string(),
                serde_json::to_value(&*combatant).unwrap(),
            );
        }
        ...
        if let Ok(equipment) = world.get::<&Equipment>(entity) {
            components.insert(
                "equipment".to_string(),
                serde_json::to_value(&*equipment).unwrap(),
            );
        }
...
mod tests {
    ...
    fn test_serialize_deserialize() {
        ...
        let new_entity = system
            .deserialize_entity(&mut new_world, serialized)
            .unwrap();
    ...
    fn test_find_by_uuid() {
        ...
        let uuid = EntityUuid::new();
        let entity = world.spawn((uuid, Name::new("Findable")));
