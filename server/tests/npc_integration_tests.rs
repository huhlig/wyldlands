...
use wyldlands_server::models::{LLMConfig, ModelManager};
...
fn test_goap_goal_satisfaction() {
    let goal = GoapGoal::new("be_at_work", "Be at work", 10).with_condition("at_work", true);
...
fn test_npc_template_creation() {
    ...
    assert_eq!(
        ...
        Some(&"merchants_guild".to_string())
    );
    assert_eq!(
        template.properties.get("shop_type"),
        Some(&"general".to_string())
    );
}

#[test]
fn test_npc_memory_marker() {
    let memory = Memory::new();
    // Memory is now a marker component, so we just verify it exists
    assert!(true);
}
...
async fn test_llm_manager_multiple_providers() {
    ...
    assert_eq!(manager.list_llm_providers().await.len(), 2);
...
fn test_llm_request_builder() {
    use wyldlands_server::models::{LLMMessage, LLMRequest};
...
fn test_behavior_type_conversion() {
    assert_eq!(
        BehaviorType::from_str("Passive"),
        Some(BehaviorType::Passive)
    );
    assert_eq!(
        BehaviorType::from_str("Wandering"),
        Some(BehaviorType::Wandering)
    );
    assert_eq!(
        BehaviorType::from_str("Aggressive"),
        Some(BehaviorType::Aggressive)
    );
...
fn test_goap_planner_cost_optimization() {
    ...
    let goal =
        GoapGoal::new("reach_b", "Reach location B", 5).with_condition("at_location_b", true);
    planner.add_goal(goal);
...
fn test_goap_planner_multiple_goals() {
    ...
    let hunger_goal =
        GoapGoal::new("satisfy_hunger", "Satisfy hunger", 10).with_condition("is_hungry", false);
    planner.add_goal(hunger_goal);

    let rest_goal = GoapGoal::new("get_rest", "Get rest", 5).with_condition("is_tired", false);
...
fn test_goap_planner_with_npc() {
    ...
    let patrol_goal =
        GoapGoal::new("patrol_area", "Patrol the area", 5).with_condition("is_patrolling", true);
...
fn test_goap_complex_dependency_chain() {
    ...
    let goal =
        GoapGoal::new("craft_goal", "Craft an item", 10).with_condition("item_crafted", true);
    planner.add_goal(goal);
