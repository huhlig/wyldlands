...
use crate::server::websocket::adapter::WebSocketAdapter;
use crate::server::{ProtocolAdapter, ProtocolMessage};
...
pub async fn handle_socket(socket: WebSocket, context: ServerContext) {
    ...
    counter!("gateway_connections_total", "server" => "websocket").increment(1);
    counter!("gateway_websocket_connections_total").increment(1);
    if config.enable_compression {
        counter!("gateway_websocket_compression_enabled_total").increment(1);
    }
    event!(Level::INFO, compression = %config.enable_compression, metric = "websocket_connection", "WebSocket connection established");
            ...
            Ok(Ok(Some(msg))) => {
                match msg {
                    ProtocolMessage::Text(text) => {
                        ...
                        // Record metrics
                        counter!("gateway_messages_received_total", "server" => "websocket")
                            .increment(1);
                        histogram!("gateway_message_size_bytes", "direction" => "received")
                            .record(text.len() as f64);
                        event!(Level::DEBUG, protocol = "websocket", size = %text.len(), metric = "message_received", "Message received");
                        ...
                        counter!("gateway_messages_sent_total", "server" => "websocket")
                            .increment(1);
                        histogram!("gateway_message_size_bytes", "direction" => "sent")
                            .record(response.len() as f64);
                        event!(Level::DEBUG, protocol = "websocket", size = %response.len(), metric = "message_sent", "Message sent");
                        ...
                        histogram!("gateway_message_processing_duration_seconds")
                            .record(duration.as_secs_f64());
                        event!(Level::DEBUG, duration_us = %duration.as_micros(), metric = "message_processing_duration", "Message processing duration");
                    }
                    ProtocolMessage::Binary(data) => {
                        ...
                        event!(Level::DEBUG, protocol = "websocket", size = %data.len(), metric = "message_received", "Message received");

                        // Handle binary data (could be compressed or server-specific)
                    ...
                    ProtocolMessage::Disconnected => {
                        ...
                        // Generate a reconnection token on logout.txt if enabled
                        if config.enable_reconnection {
    ...
    counter!("gateway_connections_closed_total", "server" => "websocket").increment(1);
    event!(
        Level::INFO,
        protocol = "websocket",
