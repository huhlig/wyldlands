...
pub use self::telnet::TermionixTelnetServer;
pub use self::webapp::router;
...
pub trait ProtocolAdapter: Send {
    ...
    ///
    /// The adapter should route this to the appropriate side channel based on
    /// client capabilities (GMCP, MSDP, WebSocket JSON, or fallback to text)
    async fn send_structured(
        &mut self,
        _output: &wyldlands_common::proto::StructuredOutput,
    ) -> Result<(), ProtocolError> {
        // Default implementation: not supported
        Err(ProtocolError::Unsupported(
            "Structured output not supported by this adapter".to_string(),
        ))
    }
}
...
pub struct ClientCapabilities {
    ...
    /// Supports MSDP server
    pub msdp: bool,

    /// Supports GMCP server
    pub gmcp: bool,
}
...
/// Helper function to convert text to server-appropriate format
