...
use wyldlands_gateway::session::{
    AuthenticatedState, ProtocolType, SessionMetadata, SessionState, UnauthenticatedState,
};
...
async fn setup_test_db() -> sqlx::PgPool {
    ...
    sqlx::query(
        ...
        CREATE TABLE sessions (
            ...
            state TEXT NOT NULL CHECK (state IN ('Unauthenticated', 'Authenticated', 'Connected', 'Disconnected')),
            server TEXT NOT NULL CHECK (server IN ('Telnet', 'WebSocket')),
    ...
    sqlx::query(
        "CREATE INDEX idx_session_command_queue_session_id ON session_command_queue(session_id)",
    )
    .execute(&pool)
    .await
...
async fn test_full_session_lifecycle() {
    ...
    manager
        ...
        .await
        .expect("Failed to transition back to Connected");
...
async fn test_session_persistence_across_restarts() {
    // Sessions are now in-memory only, so this test verifies that
    // sessions don't persist across manager instances
    let manager1 = SessionManager::new(300);
    ...
    let manager2 = SessionManager::new(300);

    // Session should not exist in new manager
    let loaded_session = manager2.get_session(session_id).await;
...
async fn test_concurrent_session_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            manager_clone
                .create_session(ProtocolType::WebSocket, format!("127.0.0.1:{}", 8000 + i))
                .await
                .expect("Failed to create session")
        });
        handles.push(handle);
...
async fn test_session_metadata_persistence() {
    ...
    session.metadata = SessionMetadata {
        ...
        side_channel_capabilities: Default::default(),
    ...
    assert_eq!(
        loaded.metadata.terminal_type,
        session.metadata.terminal_type
    );
    assert_eq!(loaded.metadata.window_size, session.metadata.window_size);
    assert_eq!(
        loaded.metadata.supports_color,
        session.metadata.supports_color
    );
    assert_eq!(
        loaded.metadata.supports_compression,
        session.metadata.supports_compression
    );
    assert_eq!(
        loaded.metadata.custom.get("theme"),
        Some(&"dark".to_string())
    );
    assert_eq!(
        loaded.metadata.custom.get("language"),
        Some(&"en".to_string())
    );
}
